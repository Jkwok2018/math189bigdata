...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = X @ W
	temp = y*np.log(np.exp(mu)/(np.exp(mu).sum(axis=1).reshape(-1, 1)))
	l2 = reg * np.diag(W.T @ W).sum()
	NLL = -temp.sum(axis=1).sum() + l2
	"*** END YOUR CODE HERE ***"
	return NLL
...
def grad_softmax(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	mu = X @ W
	temp = np.exp(mu)/(np.exp(mu).sum(axis=1).reshape(-1, 1))
	grad = X.T @ (temp - y) + reg * W
	"*** END YOUR CODE HERE ***"
	return grad
...
def predict(X, W):
	...
	mu = X @ W
	temp = np.exp(mu)/(np.exp(mu).sum(axis=1).reshape(-1, 1))
	y_pred = np.argmax(temp, axis=1).reshape(-1,1)
	"*** END YOUR CODE HERE ***"
	return y_pred
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	for reg in lambda_list:
		...
		"*** YOUR CODE HERE ***"
		W_opt, obj = grad_descent(X_train, y_train_OH, reg=reg, lr=2e-5, \
		print_freq=50)
	...
	"*** YOUR CODE HERE ***"
	max_index = np.argmax(accu_list)
	reg_opt = lambda_list[max_index]
	"*** END YOUR CODE HERE ***"
