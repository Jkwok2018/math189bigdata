...
def NLL(X, y, W, reg=0.0):
	...
	"""
	"*** YOUR CODE HERE ***"
	l2 = reg/2*np.linalg.norm(W)**2
	# in order to multiply arrays of different sizes, need to use np.multiply
	nll_0 = sum(np.multiply(y, np.log(sigmoid(X @ W)))+ np.multiply(1. -y, 
	np.log(1. -sigmoid(X @ W))))
	nll = -nll_0 + l2
	"*** END YOUR CODE HERE ***"
	return nll.item(0)
...
def newton_step(X, y, W, reg=0.0):
	...
	mu = sigmoid(X @ W)
	# have mu across the diagonal
	S = np.diag(np.squeeze(np.asarray(np.multiply(mu, 1. - mu))))
	# np.eye gives a 2D array with 1 on its diagonals and 0 everywhere else
	# input represents the number of rows in the output
...
def get_description(X, y, W):
	...
	"*** YOUR CODE HERE ***"
	y_p = predict(X, W)
	# in order to find the accuracy, we need to compare every predicted label
	# with the actual label y. Similar idea for precision and recall.
	# c = number of labels that are correctly predicted
	# t_p = number of true label being 1 given predicted is 1
	# p_p = number of predicted label that is 1
	# p_r = number of predicted label beging 1 given true label is 1
	# t_r = number of true label that is 1
	m, n = X.shape
	c, t_p, p_p, t_r, p_r = 0, 0, 0, 0, 0
	for i in range(m):
		if y.item(i) == y_p.item(i):
			c += 1
		if y.item(i) == 1: 
			t_r += 1
			if y_p.item(i):
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	reg_list = [0., 0.01, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 40.0]
	for i in reg_list:
		W_opt, obj = grad_descent(X_train, y_train, reg = i, \
			lr = 2e-4, print_freq = 100)
		result = get_description(X_test, y_test, W_opt)
		a_list.append(result[0])
		p_list.append(result[1])
		r_list.append(result[2])
	...
	"*** YOUR CODE HERE ***"
	max_index = np.argmax(a_list)
	reg_opt = reg_list[max_index]
