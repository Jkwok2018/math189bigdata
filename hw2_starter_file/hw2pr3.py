...
def linreg(X, y, reg=0.0):
	...
	equation 7.33 from Murphy shows that the w_ridge = (lambdaI+X^TX)^{-1}X^Ty
	if we manipulate this equation, we get (lambdaI+X^TX)w_ridge = X^Ty
	"""
	
	"*** YOUR CODE HERE ***"
	# I is n+1 * n+1
	lambdaI = np.eye(X.shape[1])*reg
	# in order to not regularize the basis term
	lambdaI[0,0] = 0
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	# loop through every W_opt to find RMSE
	...
	"*** YOUR CODE HERE ***"
	# find the index of the minimum regularization value
	reg_opt_index = np.argmin(RMSE_list)
	# get the value from the reg_list
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
	...
	# find the norms by caluculating the norms of each w_opt
